...
for num in "${client_nums[@]}"; do
	# 测试 b=0 的情况
	mysql-reboot
	python3 bot.py -c $num -b 0 -d uniform -r $((5000/num))
	mv ~/mysql/data/audit_log.2025-01-21.0 "uniform/no${num}.log"
	echo 生成正样例
	# 测试 b=1 的情况
	mysql-reboot
	python3 bot.py -c $((num-1)) -b 1 -d uniform -r $((5000/num))
...
for alpha in "${alphas[@]}"; do
	# 测试 b=0 的情况
	mysql-reboot
	python3 bot.py -c 100 -b 0 -d zipf -a $alpha
	mv ~/mysql/data/audit_log.2025-01-17.0 "zipf100/no${alpha}.log"

	# 测试 b=1 的情况
	mysql-reboot
	python3 bot.py -c 99 -b 1 -d zipf -a $alpha
...
for alpha in "${alphas[@]}"; do
	# 测试 b=0 的情况
	mysql-reboot
	python3 bot.py -c 500 -b 0 -d zipf -a $alpha
	mv ~/mysql/data/audit_log.2025-01-17.0 "zipf500/no${alpha}.log"

	# 测试 b=1 的情况
	mysql-reboot
	python3 bot.py -c 499 -b 1 -d zipf -a $alpha
